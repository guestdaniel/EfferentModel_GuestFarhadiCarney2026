@testset "Single- vs multi-channel" begin
    ...
    stim = pt(1000.0, 50.0)
    multichannel = sim_gfc2023_dict(pt(1000.0, 50.0), cfs; moc_weight_ic=0.0, moc_weight_wdr=0.0)
    @testset "CF = $cf Hz" for (idx_cf, cf) in enumerate(cfs)
        singlechannel = sim_gfc2023_dict(stim, [cf]; moc_weight_ic=0.0, moc_weight_wdr=0.0)
